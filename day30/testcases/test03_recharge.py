...
class RechargeTest(BaseTestCase):
    ...
    def test_recharge(self, case):
        ...
        # {"code": 0, "msg": "支付成功"}
        # pay_res = self.send_request(url=alipay_url, method='post').json()
        alipay_mock = create_autospec(self.send_request, return_value={"code": 0, "msg": "支付成功"})
        pay_res = alipay_mock(alipay_url, method='post')

        if not pay_res['code'] == 0:
